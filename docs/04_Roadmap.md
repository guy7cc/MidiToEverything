...
> **進捗**: M0〜M6 ✅ 完了（コア＋実機 MIDI/キー送信/窓監視まで結線・検証済み）。
> 動作確認バッチ: `run-midi-monitor.bat`（MIDI入力）/`run-key-test.bat`（実キー送信）/`run-window-watch.bat`（窓連動プロファイル切替）。
> 緊急停止ホットキー（`RegisterHotKey`）は WPF アプリ（M7）で実装。
> 次は M7（WPF UI: 常駐＋モニター＋可視化）。ここから GUI の目視確認が中心。
