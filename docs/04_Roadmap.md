...
> **進捗**: M0 ✅ / M1 ✅ / M2 ✅ / M3 ✅ / M4 ✅ / M5 ✅（`Win32InputSink` 実装済み）。
> 動作確認バッチ: `run-midi-monitor.bat`（MIDI入力モニタ）、`run-key-test.bat`（実キー送信→メモ帳で目視）。
> 緊急停止ホットキー（`RegisterHotKey`）はメッセージループが必要なため、Dispatcher を持つ WPF アプリ（M7）で実装する。
> 次は M6（アクティブ窓監視/プロファイル切替）。M6 もウィンドウ切替の目視確認が必要。
