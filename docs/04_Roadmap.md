...
> **進捗**: M0〜M9 ✅ 完了。コア＋実機 Infra＋WPF アプリ＋エディタ＋配布まで実装・検証済み。
> 起動: `dotnet run --project src/App/App.csproj -c Release`。配布: `publish.bat`（self-contained 単一 exe）。
> 検証: 自動起動(レジストリ登録)・発行 exe の起動・各機能をスクリーンショット/テストで確認済み（全129テスト緑）。
> 以降は B-1（MIDI 検出モード切替）等のバックログを随時。
...
**目的:** OS なしで「シグナル＋プロファイル → 有効アクション」を確定できる。**ここがアプリの心臓**。
...
- [ ] アプリシェル＋トレイ常駐（最小化でバックグラウンド, FR-7.3）、有効プロファイル/コンテキスト表示（FR-5.6）。
...
## マイルストーン M8 — プロファイルエディタ＋ラーン（2〜3日）

**目的:** GUI だけで設定を完結。

- [ ] プロファイル CRUD、match ルール編集、バインディング編集（アクション union のフォーム）。
...
**完了条件:** インストール直後から、GUI のみで CSP プロファイルを作って動かせる。
...
- [ ] self-contained single-file 発行、README（制約・既知事項）、サンプルプロファイル。
