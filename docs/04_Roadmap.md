...
> **進捗**: M0〜M8 ✅（WPF アプリ＋プロファイルエディタ＋ラーン＋インポート/エクスポートまで動作）。
> アプリ起動: `dotnet run --project src/App/App.csproj`（または発行した `MidiToEverything.exe`）。
> 検証: アプリ本体・エディタの描画、デバイス検知、設定の往復をスクリーンショット/テストで確認済み。
> 残りは M9（仕上げ・配布: 自動起動・安定性・self-contained 発行・README）。
