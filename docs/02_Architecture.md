...
│  ・ProfileManager(アクティブルール決定/切替・窓監視も購読)   │
...
- `Action`: 抽象。実装は `KeyStrokeAction`（修飾＋キー, ホールド可）, `MouseClickAction`, `CursorMoveAction`, `ScrollAction`, `SwitchProfileAction`, `MacroAction`, `NoneAction`(=何もしない/no-op)。
...
### 3.2 ルールの合算解決（`FiringEvaluator`, 純粋ロジック）
「有効なルール」をすべて合算して発火する（優先度による上書きは無い）。有効＝基本ルール（常時）＋regex がアクティブウィンドウにマッチするルール＋手動で強制 ON にしたルール（強制 OFF は除外）。`ProfileManager` が有効集合（`ActiveRules`）を決め、`FiringEvaluator` が解決する。

解決アルゴリズム（シグナル単位）:
```
evaluate(signal):
    active = [baseRule] + rules where IsActive(rule)   # IsActive = 手動override ?? regexマッチ
    fired = []
    for rule in active:
        for binding in rule.FindAllMatches(signal):    # ルール内は特異度で最上位のみ
            if binding.action is not NoneAction:        # none は不活性(no-op)
                fired.add(binding)                      # 合算（上書きしない）
    return fired                                        # 全てを発火
```
- マッチした全ルールのバインディングが**合算発火**（上書き無し）。同一シグナルを複数ルールが持てば全部発火する。
- ルール内では特異度が効く（より具体的なバインディングが同一ルール内の曖昧なものを上書き）。
- `NoneAction` は no-op で、他ルールを遮断しない（旧モデルの「ブロック」は廃止）。アプリ別に変えたいときは各ルールの regex でスコープを切る。
- 手動の強制 ON/OFF（ピン/トグル）と MIDI の「ルール切替」アクションでルールを有効/無効化できる。
...
- `System.Text.Json` で設定・ルール群を JSON 保存/ロード（スキーマは [03_ProfileSchema.md](03_ProfileSchema.md)）。
- 保存先: `%APPDATA%\MidiToEverything\` の**単一 `config.json`**（設定＋基本ルール＋全ルール＋ActiveContext を1ファイルに集約）。ログは同フォルダの `logs\`。
- 外部編集の反映: 常駐中は再読込しないが、2回目の起動で既存インスタンスが前面化される際にディスクから再読込する（`App.ReloadConfigFromDisk`。ルールエディタを開いている間はスキップ）。エディタからの保存は即時に `ProfileManager.Reload` で反映（FR-7.2）。※ `FileSystemWatcher` は未使用。
- 書き込みはテンポラリ→アトミック置換（破損防止）。`version` でマイグレーション（現行 v2、v1→v2 の自動移行あり）。

### 3.6 Presentation（WPF / MVVM）
- `DeviceListView`（一覧/接続状態）, `InputMonitorView`（ログ＋ラーン）, `VisualizerView`（ノブ/フェーダー/パッドのリアルタイム表示）, `ProfileEditorView`（バインディング編集）, `StatusBar/Tray`（有効ルール・コンテキスト表示, FR-5.6）。
